...
class DynamicIsland extends StatefulWidget {
  ...
  const DynamicIsland({Key? key, required this.message, this.isExpanded = false}) : super(key: key);
...
class _DynamicIslandState extends State<DynamicIsland> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _widthAnimation = Tween<double>(begin: 100.0, end: 250.0).animate(CurvedAnimation(
      parent: _controller,
      curve: Curves.easeInOut,
    ));
    _heightAnimation = Tween<double>(begin: 50.0, end: 100.0).animate(CurvedAnimation(
      parent: _controller,
      curve: Curves.easeInOut,
    ));
